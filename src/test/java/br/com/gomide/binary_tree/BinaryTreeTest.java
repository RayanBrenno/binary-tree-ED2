...
class BinaryTreeTest {
    ...
    void performASetOfRemovals() {
        ...
        binaryTreeOps.remove(rootNode, 800);
        assertEquals(binaryTreeOps.toString(rootNode), "root:100 (left:90 (left:30 (left:5 )))");
    ...
    void dontFindRequiredElement() {
        ...
        binaryTreeOps.remove(rootNode, 4);
    ...
    void checkNodeDegree() {
        ...
        binaryTreeOps.remove(rootNode, 1);
        assertEquals(binaryTreeOps.degree(rootNode, 2), 1);

        binaryTreeOps.remove(rootNode, 6);
        assertEquals(binaryTreeOps.degree(rootNode, 8), 1);
    }

    @Test
    void getFather() {
        IBinaryTree<Integer> binaryTreeOps = new BinaryTree<>();
        Integer[] elements = new Integer[] { 6, 2, 8, 1, 4, 3, 9 };
        ...
        assertEquals(binaryTreeOps.getFather(rootNode, 9).getValue(), 8);

        binaryTreeOps.remove(rootNode, 6);
        assertEquals(binaryTreeOps.getFather(rootNode, 2).getValue(), 8);
    ...
    void getBrother() {
        ...
        binaryTreeOps.remove(rootNode, 4);
        assertEquals(binaryTreeOps.getBrother(rootNode, 3).getValue(), 1);

        binaryTreeOps.remove(rootNode, 8);
        assertEquals(binaryTreeOps.getBrother(rootNode, 2).getValue(), 9);

        binaryTreeOps.remove(rootNode, 6);
        assertNull(binaryTreeOps.getBrother(rootNode, 2));
    ...
    void calculateNodeLevel() {
        ...
        assertEquals(binaryTreeOps.calculateNodeLevel(rootNode, 2), 1);

        binaryTreeOps.remove(rootNode, 4);
        assertEquals(binaryTreeOps.calculateNodeLevel(rootNode, 3), 2);
